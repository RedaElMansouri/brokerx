...
(function() {
    ...
    if (el) el.textContent = name + ' (Microservices)';
    } catch (e) {}
})();
...
// ---- UC-04: Intégration temps réel via ActionCable (via Orders Service) ----
(function() {
    const token = localStorage.getItem('auth_token');
    ...
    try {
        ...
        // Connect to Orders Service WebSocket (port 3003)
        const wsURL = 'ws://localhost:3003/cable?token=' + encodeURIComponent(token);
...
// ---- Notifications: subscribe to per-user OrdersChannel ----
(function(){
    const token = localStorage.getItem('auth_token');
    if (!token) return;
    function ensureCable(cb) {
